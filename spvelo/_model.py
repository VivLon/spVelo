...
from scvi.data.fields import LayerField, StringUnsField, CategoricalObsField, ObsmField
...
class SPVELO(VAEMixin, UnsupervisedTrainingMixin, BaseModelClass):
    """spVelo model

    Parameters
    ----------
    adata
        AnnData object that has been registered via :func:`~spvelo.SPVELO.setup_anndata`.
    n_hidden
    ...
    **model_kwargs
        Keyword args for :class:`~spvelo.SPVELOVAE`
    """
    ...
    def __init__(
        ...
        **model_kwargs,
        ...
        self.module = SPVELOVAE(
            penalty=penalty,
            add_mnn=add_mnn,
            niche_edge=niche_edge,
            subsample=subsample,
        ...
        self._model_summary_string = (
            "SPVELO Model with the following params: \nn_hidden: {}, n_latent: {}, n_layers: {}, dropout_rate: "
            "{}"
        ).format(
            n_hidden,
            n_latent,
    ...
    def get_state_assignment(
        ...
        if n_samples > 1 and return_mean is False:
            if return_numpy is False:
                warnings.warn(
                    "return_numpy must be True isf n_samples > 1 and return_mean is False, returning np.ndarray"
                )
            return_numpy = True
        if indices is None:
    ...
    def get_MVG_param(
        ...
        """Return qz_m and qz_v before or after adding spatial location.
        """
        adata = self._validate_anndata(adata)
    ...
    def setup_anndata(
        ...
        **kwargs,
        ...
        anndata_fields = [
            ...
            LayerField(REGISTRY_KEYS.INDEX_KEY, index_data, is_count_data=False),
            CategoricalObsField(REGISTRY_KEYS.BATCH_KEY, batch_data),
            CategoricalObsField(REGISTRY_KEYS.LABELS_KEY, label_data),
            ObsmField(REGISTRY_KEYS.SPATIAL_KEY, spatial_data),
        ]
